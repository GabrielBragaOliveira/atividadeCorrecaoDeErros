...
public class FuncionarioService {
    ...
    public FuncionarioResponse atualizar(Long id, FuncionarioRequest req) {
        ...
        if (Boolean.FALSE.equals(existente.getAtivo())) {
            throw new BusinessException("Não é possível atualizar um funcionário inativo");
        }
    ...
    public record Result<T>(T body, boolean created, boolean reactivated) {
        ...
        public static <T> Result<T> created(T body) {
            return new Result<>(body, true, false);
        }

        public static <T> Result<T> reactivated(T body) {
            return new Result<>(body, false, true);
        }
    }
}
